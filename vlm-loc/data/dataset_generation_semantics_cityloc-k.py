...
from data.utils import project_points_to_pixels
...
def create_train_info(scene_info):
    ...
    # image_path.replace("./data", "/data/kang/vlmloc/data")
...
def ground_pose_to_image_scene_graph(pose, scene_graph, objects_in_cell, object_instance_ids) -> List[Dict[str, Any]]:
    ...
    Matching rule:
      ...
      2) If the semantic label does not match, it is considered unmatched.
      3) If the label matches, iterate over all candidate nodes of that label,
         compute distances from all object points to the node, and select the
         node with the minimum distance if it is within the threshold.
    """
...
def ground_pose_to_image_scene_graph_v2(pose, scene_graph, objects_in_cell, object_instance_ids) -> List[Dict[str, Any]]:
    ...
    Matching rule:
      ...
      2) If the semantic label does not match, it is considered unmatched.
      3) If the label matches, iterate over all candidate nodes of that label,
         compute distances, and select the closest node if it is within the threshold.
    """
    grounded_info: List[Dict[str, Any]] = []
    ...
    for desc in getattr(pose, "descriptions", []):
        ...
        pose_obj_center_world = pose_obj_center_incell * BEV_RANGE + np.array(pose.pose_w[0:2]) - BEV_RANGE / 2
...
def create_text_image_pairs(
    ...
    for pid, pose in tqdm(enumerate(poses), desc=f"Creating text-video pairs for scene {scene}", total=len(poses)):
        ...
        # Project pose world coordinates to image pixels
        ...
        # Export as string for training/output
...
def process_single_scene(scene, args, BEV_RANGE, IMAGE_SIZE, OUTPUT_DIR, DATA_DIR):
    """Process a single scene for multithreaded generation."""
...
if __name__ == "__main__":
    ...
    GENERATION_SPLIT = SCENE_NAMES_VAL
    # GENERATION_SPLIT = SCENE_NAMES_TRAIN
    ...
    # NEED TO SPECIFY
    DATA_DIR = '/home/data_sata/vlmloc/Text2Loc/data/k360_50-10_gridCells_pd10_pc2_shiftPoses_all_nm-6_v5'
    # Output parameters
    output_version = "vlmloc_data_v5_0310_test"
    OUTPUT_DIR = os.path.join("/home/data_sata/vlmloc/vlm-loc", output_version)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_scene):
            ...
            try:
                ...
                print(f"✗ Scene {scene} generated an exception: {exc}")
    ...
    print(f"Saved {len(training_data)} training samples to {training_json_path}")
