...
def get_direction(obj, pose, on_top_thr=0.05):
    """
    根据 pose 与物体的相对位置关系，返回方向标签：
      - 若 pose 距离物体中心 < on_top_thr → "on-top"
      - 否则根据物体整体中心与 pose 的相对位置计算主方向。
    """
    closest_point = obj.get_closest_point(pose)
    obj2pose = pose - closest_point

    if np.linalg.norm(obj2pose[:2]) < on_top_thr:
        return "on-top"

    if hasattr(obj, "xyz") and obj.xyz.shape[0] > 0:
        center = np.mean(obj.xyz[:, :3], axis=0)
