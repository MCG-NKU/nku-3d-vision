...
#     "Water",
# ]
from datapreparation.kitti360pose.select_ori import (
    get_direction,
...
def cluster_stuff_object(obj, stuff_min, eps=0.75, map_to_raw=True):
    """
    Perform DBSCAN clustering on (downsampled) obj.xyz,
    and optionally map cluster assignments back to obj.xyz_raw for consistency.
    Return both downsampled and raw clustered objects.
    """
    if obj.xyz.shape[0] == 0:
        return []
    ...
    # 2. map xyz to xyz_raw in xy-plane
    if map_to_raw and hasattr(obj, "xyz_raw"):
        tree = KDTree(obj.xyz[:, :2])  
        _, nn_idx = tree.query(obj.xyz_raw[:, :2], k=1)
        raw_labels = labels[nn_idx[:, 0]]
    else:
        raw_labels = labels

    # 3. generate clustered objects based on labels
    for cluster_id in range(0, np.max(labels) + 1):
        mask_ds = labels == cluster_id
        mask_raw = raw_labels == cluster_id
        if np.sum(mask_ds) >= stuff_min:
            c_obj_ds = obj.mask_points(mask_ds, use_raw=False)
            c_obj_ds = c_obj_ds.mask_points(mask_raw, use_raw= True)
            clustered_objs.append(c_obj_ds)

        # ========== 原始点云版 ==========
        # mask_raw = raw_labels == cluster_id
        # if np.sum(mask_raw) >= stuff_min:
        #     c_obj_raw = obj.mask_points(mask_raw, use_raw=True)
        #     clustered_raw.append(c_obj_raw)

    return clustered_objs
...
def create_cell(
    ...
    num_mentioned=6,
    inside_fraction=1 / 3,
    stuff_min=1000, # 1000 for CityLoc as the range is 50x50
    all_cells=False,
    stuff_classes=None,
) -> Cell:
    ...
    for obj in scene_objects:
        ...
        if not obj.xyz.shape[1] == 3:
            print(obj.xyz.shape)
        mask = get_mask(obj.xyz, bbox_w)
        mask_raw = get_mask(obj.xyz_raw, bbox_w)
        if obj.label in stuff_classes:
            if np.sum(mask) < stuff_min:
                continue

            cell_obj = obj.mask_points(mask, use_raw=False)
            cell_obj = cell_obj.mask_points(mask_raw, use_raw=True)
