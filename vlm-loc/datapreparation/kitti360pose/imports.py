...
class Object3d:
    ...
    def __init__(self, id: int, instance_id: int, xyz: np.ndarray,rgb: np.ndarray,label: str, xyz_raw= None, rgb_raw=None):
    ...
    def mask_points(self, mask, use_raw):
        ...
        if use_raw:
            ...
            assert self.xyz_raw.shape[1] == 3
            assert mask.shape[0] == self.xyz_raw.shape[0], f"Shape mismatch: mask.shape[0]={mask.shape[0]}, xyz_raw.shape[0]={self.xyz_raw.shape[0]}"
            # return Object3d(self.xyz[mask], self.rgb[mask], self.label, self.id)
            return Object3d(self.id, self.instance_id, self.xyz, self.rgb, self.label, self.xyz_raw[mask], self.rgb_raw[mask])
        else:
            assert len(mask) > 6  # To prevent bbox input
            assert self.xyz.shape[1] == 3
            assert mask.shape[0] == self.xyz.shape[0], f"Shape mismatch: mask.shape[0]={mask.shape[0]}, xyz.shape[0]={self.xyz.shape[0]}"

            # return Object3d(self.xyz[mask], self.rgb[mask], self.label, self.id)
            return Object3d(self.id, self.instance_id, self.xyz[mask], self.rgb[mask], self.label, self.xyz_raw, self.rgb_raw)
    ...
    def merge(cls, obj1, obj2):
        ...
        return Object3d(
            ...
            obj1.label,
            np.vstack((obj1.xyz_raw, obj2.xyz_raw)),
            np.vstack((obj1.rgb_raw, obj2.rgb_raw)),
        )
...
class Pose:
    def __init__(
        ...
        eval_cell_id: int=None,
        eval_scene_name: str=None,
        ...
        self.eval_cell_id = eval_cell_id  # ID of the eval cell in the database
        self.eval_scene_name = eval_scene_name
